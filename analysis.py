...
# Reusable functions
def filter_quality(properties: List[Dict[str, Any]],
                   geometries: List[Dict[str, Any]],
                   ids: List[str],
                   min_points: int = 5,
                   min_view_angle: float = 3) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[str]]:
    """Filter out scenes with insufficient points or bad quality."""
    index_sub = [i for i, prop in enumerate(properties)
                 if prop.get('ground_control', False) and
                 prop.get('quality_category', '') == 'standard' and
                 prop.get('view_angle', 0) < min_view_angle and
                 len(geometries[i]['coordinates'][0]) >= min_points]

    return ([properties[i] for i in index_sub],
            [geometries[i] for i in index_sub],
            [ids[i] for i in index_sub])

def compute_central_coordinates(geometries: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray]:
    """Compute central latitude and longitude for each polygon."""
    central_lon = np.array([np.mean([pt[0] for pt in geom['coordinates'][0]]) for geom in geometries])
    central_lat = np.array([np.mean([pt[1] for pt in geom['coordinates'][0]]) for geom in geometries])
    return central_lon, central_lat

def compute_local_times(properties: List[Dict[str, Any]], central_lon: np.ndarray) -> np.ndarray:
    """Compute local times from UTC and central longitude."""
    hours = np.array([int(p['acquired'].split('T')[1].split('Z')[0].split(':')[0]) for p in properties])
    minutes = np.array([int(p['acquired'].split('T')[1].split('Z')[0].split(':')[1]) for p in properties])
    seconds = np.array([float(p['acquired'].split('T')[1].split('Z')[0].split(':')[2]) for p in properties])
...
def calculate_intersections(polygons: List[Polygon],
                            properties: List[Dict[str, Any]],
                            min_sun_angle: float = 0,
                            area_threshold: float = 25.0) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if instruments[i] == instruments[j] and satellite_ids[i] != satellite_ids[j]:
                if polygons[i].bounds[2] >= polygons[j].bounds[0] and \
                   polygons[i].bounds[3] >= polygons[j].bounds[1]:
...
def process_tiles(all_properties: List[List[Dict[str, Any]]],
                  all_geometries: List[List[Dict[str, Any]]],
                  all_ids: List[List[str]],
                  min_view_angle: float = 3,
                  min_sun_angle: float = 0) -> gpd.GeoDataFrame:
    """
    ...
    for props, geoms, ids in zip(all_properties, all_geometries, all_ids):
        f_props, f_geoms, f_ids = filter_quality(props, geoms, ids, min_view_angle=min_view_angle)
    ...
    area_2d, sun_diff_2d = calculate_intersections(polygons, merged_properties, min_sun_angle=min_sun_angle)
